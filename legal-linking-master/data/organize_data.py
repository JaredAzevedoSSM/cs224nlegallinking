...
Desc: convert json data into data with input, matching sentence, and label
"""
import json
import os
import sys
...
def load(inputpath, cut):
    """
    Name: load
    Desc: load all preferred files into single list
    """
    files = [x for x in os.listdir(inputpath) if cut in x]
    data = []
...
def organize_and_label(data):
    """
    Name: organize_and_label
    Desc: organize each example so that is only contains the properties we care about and add label
    """
    organized_data = []

    for example in data:
        formatted = {'input': example['text'], 'match': example['matches'][0][0] if len(example['matches']) > 0 else "None", 'label': 0 if len(example['matches']) == 0 else 1}
        organized_data.append(formatted)
...
def separate(data):
    """
    Name: separate
    Desc: break the properties we gathered into separate lists so they can be easily converted to pandas dataframe
    """
    inputs = []
    matches = []
    labels = []
